...
class EosInter:
  def __init__(self, fname):
    ...
    self.lnx11d = np.log(
        self.eosf.block[0]['x1'].data + self.eosf.block[0]['x1shift'].data).squeeze()
    self.lnx21d = np.log(
        self.eosf.block[0]['x2'].data + self.eosf.block[0]['x2shift'].data).squeeze()
  ...
  def _prep(self, rho, ei):
    ...
    i1 = ne.evaluate(
        "(lnx1 - x1off) * x1fac").astype(np.int32).clip(0, min(nx1, self.n1))
    i2 = ne.evaluate(
        "(lnx2 - x2off) * x2fac").astype(np.int32).clip(0, min(nx2, self.n2))
