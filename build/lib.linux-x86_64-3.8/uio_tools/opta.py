...
class Opac:
  def __init__(self, fname):
    ...
    tabKap = np.array(
        [a.strip() for sublist in tabKap for a in sublist if a != '']).astype(np.float32)
  ...
  def tau(self, rho, axis=-1, **kwargs):
    ...
    if 'kappa' in kwargs:
      kaprho = ne.evaluate(
          "kappa*rho", local_dict={'rho': rho, 'kappa': kwargs['kappa']})
  ...
  def height(self, z, value=1.0, axis=-1, **kwargs):
    ...
    if 'rho' in kwargs:
      if 'kappa' in kwargs:
        ...
        if 'iBand' in kwargs:
          tau = self.tau(kwargs['rho'], axis=axis, P=kwargs['P'],
                         T=kwargs['T'], zb=zb, iBand=kwargs['iBand'])
        else:
          tau = self.tau(kwargs['rho'], axis=axis,
                         P=kwargs['P'], T=kwargs['T'], zb=zb)
      else:
    ...
    if 'tau' in kwargs:
      ...
      if 'rho' in kwargs:
        ...
        if 'zb' in kwargs:
          ...
          raise ValueError(
              "Either 'zb' or 'z' has to be provided, if 'tau' is not available.")
        ...
        if 'kappa' in kwargs:
          ...
          tau = self.tau(kwargs['rho'], axis=axis,
                         kappa=kappa, zb=zb, radhtautop=radhtautop)
        elif 'T' in kwargs and 'P' in kwargs:
          ...
          if 'iBand' in kwargs:
            ...
            tau = self.tau(kwargs['rho'], axis=axis, P=P,
                           T=T, zb=zb, radhtautop=radhtautop)
        else:
          raise ValueError("Either the keyword-argument 'kappa', or 'T' (temperature) and 'P' (pressure) have"
                           " to be provided.")
      else:
        raise ValueError(
            "Either the keyword-argument 'tau' or 'rho' has to be provided.")
    ...
    if dim == 3:
      if type(ntau) == np.ndarray:
        if ntau.ndim == 1:
          ...
          raise ValueError(
              "new_tau has wrong dimension. new_tau.ndim must be 1, 2, or 3")
      else:
      ...
      if type(ntau) == np.ndarray:
        if ntau.ndim == 1:
          ...
          raise ValueError(
              "new_tau has wrong dimension. new_tau.ndim must be 1, 3, or 4")
      else:
        # result will be reduced by 1 dimension (plane). Trans has to be redefined.
        if axis == 0:
          trans = [2, 0, 1]
      ...
      raise ValueError(
          "tau has wrong dimension. new_tau.ndim must be 1, 3, or 4")
